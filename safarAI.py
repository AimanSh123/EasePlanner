...
# Set page configuration for tab title and icon
st.set_page_config(page_title="EasePlanner", page_icon="🗺️")

# Define colors directly (for example, using Light Mode colors)
primary_color = "#FFFFFF"
secondary_color = "#F0F0F0"
text_color = "#000000"
button_gradient = "linear-gradient(to right, #E55D87 0%, #5FC3E4 51%, #E55D87 100%)"
...
# Display the logo
st.image("logo/easeplanner.png", use_column_width=False, width=105)

# Title and Subtitle
st.markdown('<div class="subtitle">Let\'s plan your next trip</div>', unsafe_allow_html=True)
...
if not st.session_state['checklist']:
    with placeholder.form(key='travel_form'):
        ...
        st.markdown('<div class="question">What is your budget for this trip (in INR/₹)?</div>', unsafe_allow_html=True)
    ...
    if submit_button:
        ...
        if missing_fields:
            ...
            prompt = (
                ...
                f"The group size is {group_size} with a budget of {budget} Indian National Rupees. It's {season}. Special considerations: {special_considerations}."
            )
            try:
...
if st.session_state['checklist']:
    ...
    download_button = st.download_button(
        ...
        data=generate_pdf(st.session_state['checklist'], "logo/easeplanner.png"),
...
st.markdown('<div class="footer">All rights reserved | Made with ❤️ by Aiman Shaikh</div>', unsafe_allow_html=True)